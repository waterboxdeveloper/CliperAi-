...
## Mejora 2025-12-07: Cleanup Específico y Exhaustivo

### Cambios Realizados

**Problema Identificado:**
- Cleaner borraba directorios, pero dejaba **caché y archivos residuales**
- FFmpeg podía dejar temp files que interferían con futuras ejecuciones
- Python dejaba `__pycache__` que podía corromper imports
- macOS dejaba `.DS_Store` que interfería en búsquedas

**Solución Implementada:**

Nuevo método `_clean_cache_and_residuals()` que limpia específicamente:

```
1. Archivos .lock en temp/ (pueden bloquear subsecuentes)
2. temp_*.mp4, temp_reframed_*.mp4 (residuales de video_exporter)
3. SRTs huérfanos (sin .mp4 correspondiente)
4. __pycache__/ en todo el proyecto (caché Python)
5. .DS_Store en macOS (caché de Finder)
```

**Integración:**

```python
# Se ejecuta automáticamente en delete_all_project_data()
# SIEMPRE se ejecuta, antes de reset de state

results['cache'] = self._clean_cache_and_residuals()
```

**Resultado:**
- Cleaner es ahora ESPECÍFICO y exhaustivo
- No quedan archivos que puedan contaminar futuras ejecuciones
- Logging detallado de qué se limpió y cuánto espacio se liberó
...
**Test 3: Cleanup Total (CON CACHE CLEANUP)**
...
- [ ] Cache limpiado:
  - [ ] No hay archivos .lock
  - [ ] No hay temp_*.mp4 residuales
  - [ ] No hay __pycache__ en proyecto
  - [ ] No hay .DS_Store
- [ ] State = {}
- [ ] Menu muestra "No videos found"
- [ ] Siguiente ejecución no encuentra artifacts viejos
...
- [ ] Archivos lock presentes: se eliminan correctamente
- [ ] temp_*.mp4 presentes: se eliminan correctamente
